...
console.log(soma.reduce((acc, curr) => acc + curr, 0))

// Exercício 2 – Contar ocorrências de uma palavra
const respostas = ['sim', 'não', 'sim', 'sim', 'não', 'sim'];
console.log(respostas.reduce((acc, palavra) => palavra === 'sim' ? acc + 1 : acc, 0))

// Exercício 3 – Transformar array em objeto
// Objetivo: Transforme o array em um objeto onde a chave é o número e o valor é seu quadrado.
const numeros2 = [1, 2, 3, 4];
const objetoQuadrados = numeros2.reduce((obj, curr) => {
   obj[curr] = curr * curr
   return obj;
}, {})
...
const arraysInArray = arrays.reduce((acc, curr) => acc.concat(curr), [])
...
const contaLetras = [...palavra].reduce((obj, num) => {
   obj[num] = (obj[num] || 0) + 1
   return obj;
}, {})
