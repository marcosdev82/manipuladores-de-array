...
const transforme = numeros2.flatMap((arr) => {
    return [[arr, arr * 2]]
})
...
Object.entries(produto).forEach(([index, value]) => {
...
const normalizar = pedidos.flatMap((pedido)=> {
    return pedido.produtos.map((produto) => ({
        pedidoId: pedido.id,
        produto: produto
    }))
...
console.log(dados.filter(([index,pessoa]) => index !== 2 ))

// 5. Encontre a Maria usando o Array bidimencional
console.log(dados.find(([index, nome]) => nome === 'Maria'))
...
const userPedidos = produtos.map((produto) => {
    const pessoa = usuarios.find((user) => produto.userId === user.id)
    return {...produto, 'usuario': pessoa.nome}
    // return pessoa;
})
